...
extension ToastView {
  ...
  private func _layoutContentViews() {
    ...
    // Layout components.
    _components.forEach { [unowned self] in
      $0.layout(
        in: self._contentView,
        provider: self
      )
    }
    
    let point = _components.reduce((0.0, 0.0)) {
      (min($0.0, $1.frame.minX - $1.layout.insets.left),
